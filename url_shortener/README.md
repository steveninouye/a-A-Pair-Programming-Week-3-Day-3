...
generating a random, easy to remember, code and storing this code as the `short_url` in our table. Now, we can search for this record
...
it to the database should use the `ShortenedUrl::random_code` method to provide a unique `short_url`. Make sure that each `ShortenedUrl` has a unique and random `short_url` code.
